...
if (!mongoUri) {
    console.log("⚠️  MONGO_URI not specified in environment variables.");
    console.log("⏳ Launching mongodb-memory-server for zero-config local development...");
    try {
        const { MongoMemoryServer } = await import("mongodb-memory-server");
        const mongoServer = await MongoMemoryServer.create();
        mongoUri = mongoServer.getUri();
        process.env.MONGO_URI = mongoUri;
        console.log(`✨ In-memory MongoDB started successfully! URI: ${mongoUri}`);
    } catch (err) {
